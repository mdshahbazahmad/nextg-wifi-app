...
window.supportDeskState = window.supportDeskState || {
    activeTickets: [
        { 
            id: "TKT-10492", 
            category: "Red Light Blinking", 
            description: "Optical red light blinking on main router.", 
            status: "In-Progress", 
            date: "13 Sep, 2026", 
            assignedTo: "Rahul (Field Engineer)" 
        }
    ]
};
...
function renderSupportDeskComponent(containerElement) {
    ...
    containerElement.innerHTML = `
        <div class="support-wrapper">
            ...
            <div class="support-card">
                <div class="history-header">
                    ...
                    <span class="count-badge" id="ticketCountBadge">${state.activeTickets.length} Active</span>
                </div>

                <div class="ticket-list" id="ticketListContainer">
                    ${renderTicketItems(state.activeTickets)}
                </div>
            </div>
        </div>
        ...
        <style>
            .support-wrapper { color: #fff; text-align: left; font-family: system-ui, -apple-system, sans-serif; }
            ...
            .modal-overlay { display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0, 0, 0, 0.7); align-items: center; justify-content: center; z-index: 9999; }
...
// Helper: Render Ticket Items HTML
function renderTicketItems(tickets) {
    if (!tickets || tickets.length === 0) {
        return `<p style="font-size: 12px; color: #64748b; text-align: center; padding: 10px;">No open support tickets found.</p>`;
    }
    return tickets.map(tkt => `
        <div class="ticket-item">
            <div class="tkt-top">
                <strong>${tkt.category}</strong>
                <span class="tkt-status ${tkt.status.toLowerCase().replace(/\s+/g, '-')}">${tkt.status}</span>
            </div>
...
function bindSupportDeskEvents(container) {
    ...
    if (form) {
        form.addEventListener('submit', (e) => {
            ...
            // Directly post to Operator Portal (Simulated Backend Dispatch)
            console.log("--> Dispatched to Universal Operator Admin Portal:", newTicket);

            // Dynamic Update List & Counter in UI
            ...
            if (ticketContainer) ticketContainer.innerHTML = renderTicketItems(window.supportDeskState.activeTickets);
            if (countBadge) countBadge.textContent = `${window.supportDeskState.activeTickets.length} Active`;
            ...
            if (modalMsg) {
                modalMsg.innerHTML = `Ticket <strong>#${newTicketId}</strong> has been created and sent directly to the Operator Admin Portal for immediate resolution.`;
            }
