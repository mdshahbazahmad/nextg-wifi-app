...
export function initNewConnectionModule() {
  // ID mismatch fix: View container ko target kar rahe hain
  const container = document.getElementById('newConnectionOffersContainer') || document.getElementById('view-newconn');
  
  if (!container) return;

  // HTML content create karna
  const cardsHtml = sampleConnectionOffers.map(offer => `
    <div class="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-md border dark:border-gray-700 space-y-3 relative my-3">
  ...
  // Container mein inject karna (Header ke saath)
  container.innerHTML = `
    <h3 class="text-sm font-bold text-gray-900 dark:text-white flex items-center gap-1.5 mb-3">
      <i data-lucide="plus-circle" class="w-4 h-4 text-emerald-500"></i> New Connection Offers
    </h3>
    ${cardsHtml}
  `;
  ...
  // 2. Apply Now बटन और Modal Logic
